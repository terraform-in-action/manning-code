...
locals {
  ...
  steps = [
    ...
    },
    {
      ...
      args = ["beta", "run", "deploy", google_cloudrun_service.service.name, "--image", local.image, "--region", var.region, "--platform", "managed", "-q"]
    }
  ]
}
...
resource "google_cloudrun_service" "service" {
  depends_on = [google_project_service.enabled_service["cloudrun.googleapis.com"]]
...
resource "null_resource" "cloudrun_allow" {
  provisioner "local-exec" {
    command = <<EOF
cd ${path.module}
gcloud beta run services set-iam-policy ${google_cloudrun_service.service.name} --region us-central1 policy.yaml -q --project ${var.project_id}
EOF
  }
}

data "shell_script" "cloudrun_url" {
  working_directory = path.module
  lifecycle_commands {
    read = <<EOF
sleep 10
URL=$(gcloud beta run services describe ${google_cloudrun_service.service.name} --platform managed --region ${var.region} --project ${var.project_id} --format "value(status.url)")
echo '{"url": "'"$URL"'"}' >&3
EOF
  }
}
